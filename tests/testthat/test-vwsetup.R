...
test_that("vwsetup correctly setup model with different learning modes", {
  # Empty setup
  expect_identical(vwsetup(dir = "../my_tmp/", model = "mdl.vw"), test_model)
  
  # Reference test model for nn mode
  nn_test_model <- test_model
  nn_test_model$params$options = list(nn = list(hidden=3,
                                           inpass=FALSE,
                                           multitask=FALSE,
                                           dropout=FALSE,
                                           meanfield=FALSE))
  ...
  expect_identical(
    vwsetup(dir = "../my_tmp/", model = "mdl.vw", option = "nn"),
    nn_test_model
  )
  
  # Reference test model for lda mode
  lda_test_model <- test_model
  lda_test_model$params$options = list(lda = list(num_topics=0,
                                           ...
                                           math_mode=0,
                                           minibatch=1))
  lda_test_model$params_str = paste0("--lda 0")
  
  expect_identical(
    vwsetup(dir = "../my_tmp/", model = "mdl.vw", option = "lda"),
    lda_test_model
  )
  ...
  custom_test_model$params$options = list(binary = list())
